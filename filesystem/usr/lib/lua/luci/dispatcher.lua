			...
			if authen then
				...
				if not user or not util.contains(accs, user) then
					...
					if not sess then
						...
						if sdat then
                            local f=io.open("/tmp/stok_appuid.map","r")
                            if f ~= nil then
                                io.close(f)
                                tmp = luci.util.exec("awk -F\'=\' \'{print $2}\' /tmp/stok_appuid.map")
                                token = string.sub(tmp, 1, 32)
                            else
                                token = sys.uniqueid(16)
                                os.execute("echo "..token.." > /tmp/token.dat")
                            end
...
function createtree()
	if not index then
        ...
        if (fs.stat(indexcache, "size") == 0) then
            os.execute("echo \"[APRO][WEB] indexcache broken and remove it.\" > /dev/kmsg")
